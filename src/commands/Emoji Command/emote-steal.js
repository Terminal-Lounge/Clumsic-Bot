...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('emote-steal')
        .setDescription('Steal an Emote from a message link')
        .addStringOption(option =>
            option.setName('message_link')
                .setDescription('The link to the message with the Emote')
                .setRequired(true)),
    async execute(interaction) {
        ...
        try {
            ...
            if (!emojiMatch) {
                await interaction.reply({ content: 'No Emote found in the message.', ephemeral: true });
                return;
            }
            ...
            console.error(error);
            await interaction.reply({ content: 'Failed to steal Emote.', ephemeral: true });
        }
    },
};